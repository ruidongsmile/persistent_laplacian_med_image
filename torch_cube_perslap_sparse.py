...
def torch_boundary_opr_coo_sparse(X, channel, filt, cell_dim=2, device="cpu"):
    c, num_v, num_e, num_s = torch_chains_count(X)
    chain_space_dim = torch_chains_count(X)[cell_dim]
    cell_coords = torch_cell_filt_v2(X, channel=channel, filt=filt, cell_dim=cell_dim)
    bdry_coords = torch_bdry_idx_v2(X=X, channel=channel, cell_coords=cell_coords, cell_dim=cell_dim, device=device)
    ...
    if cell_dim == 0:
        # bdry_opr = torch.zeros(1, num_v).to(dtype=torch.float32, device=device)
        brdy_opr = torch.sparse_coo_tensor((1, num_v), dtype=torch.float32, device=device)
    else:
        if len(bdry_coords) != 0:
            num_cells, bdry_cells = bdry_coords.shape
            which_column = torch.tensordot(torch.arange(num_cells), torch.ones(bdry_cells).to(int), dims=0).to(device)
            bdry_indices = torch.stack([bdry_coords.flatten(), which_column.flatten()]).to(int)
            # bdry_opr = torch.zeros([chain_space_dim, num_cells]).to(dtype=torch.float32, device=device)
            if cell_dim == 2:
                val = torch.tensor([-1., 1., -1., 1.]).repeat(num_cells).to(dtype=torch.float32, device=device)
                bdry_opr = torch.sparse_coo_tensor(bdry_indices, val, (chain_space_dim, num_cells))
                # bdry_opr[bdry_indices[:, 0], bdry_indices[:, 1]] = torch.tensor([-1., 1., -1., 1.]).repeat(num_cells).to(dtype=torch.float32, device=device)
            elif cell_dim == 1:
                # bdry_opr[bdry_indices[:, 0], bdry_indices[:, 1]] = torch.tensor([-1., 1.]).repeat(num_cells).to(dtype=torch.float32, device=device)
                val =  torch.tensor([-1., 1.]).repeat(num_cells).to(dtype=torch.float32, device=device)
                bdry_opr = torch.sparse_coo_tensor(bdry_indices, val, (chain_space_dim, num_cells))
        else:
            bdry_opr = torch.sparse_coo_tensor((chain_space_dim, 1), dtype=torch.float32, device=device)
            # bdry_opr = torch.zeros(chain_space_dim, 1).to(dtype=torch.float32, device=device)
    return bdry_opr # a tensor object
...
def torch_laplacian_up_coo_sparse(X, channel, filt, cell_dim=2, device="cpu"):
    if cell_dim == 0:
        bdry_opr_1 = torch_boundary_opr_coo_sparse(X, channel=channel, filt=filt, cell_dim=1, device=device) #.to(torch.float32)
        
        bdry_opr_1 = bdry_opr_row_simplyfying(bdry_opr_1)
        lap_up = torch.mm(bdry_opr_1, torch.transpose(bdry_opr_1, 0, 1))
        
    elif cell_dim == 1:
        bdry_opr_2 = torch_boundary_opr_coo_sparse(X, channel=channel, filt=filt, cell_dim=2, device=device) #.to(torch.float32)
        bdry_opr_2 = bdry_opr_row_simplyfying(bdry_opr_2)  # bdry_opr_2[~torch.all(bdry_opr_2 == 0, axis=1)]
        lap_up = torch.mm(bdry_opr_2, torch.transpose(bdry_opr_2, 0, 1))
        
    elif cell_dim == 2:
        len_dim_2 = len(torch_cell_filt_v2(X, channel=channel, filt=filt, cell_dim=2))
        lap_up = torch.sparse_coo_tensor((len_dim_2, len_dim_2), device=device)
        # torch.zeros([len_dim_2, len_dim_2]).to(dtype=torch.float32, device=device)

    if len(lap_up) == 0:
        lap_up = torch.tensor([0]).to(dtype=torch.float32, device=device)
    return lap_up

def torch_laplacian_down_coo_sparse(X, channel, filt, cell_dim=2):
    if cell_dim == 0:
        len_dim_0 = len(torch_cell_filt_v2(X, channel=channel, filt=filt, cell_dim=0))
        lap_down = torch.sparse_coo_tensor((len_dim_0, len_dim_0))  # torch.zeros([len_dim_0, len_dim_0])
        
    elif cell_dim == 1:
        bdry_opr_1 = torch_boundary_opr_coo_sparse(X, channel=channel, filt=filt, cell_dim=1)
        bdry_opr_1 = bdry_opr_row_simplyfying(bdry_opr_1)  # bdry_opr_1[~torch.all(bdry_opr_1 == 0, axis=1)]
        lap_down = torch.mm(torch.transpose(bdry_opr_1, 0, 1), bdry_opr_1)
        
    elif cell_dim == 2:
        bdry_opr_2 = torch_boundary_opr_coo_sparse(X, channel=channel, filt=filt, cell_dim=2)
        bdry_opr_2 = bdry_opr_row_simplyfying(bdry_opr_2)  # bdry_opr_2[~torch.all(bdry_opr_2 == 0, axis=1)]
        lap_down = torch.mm(torch.transpose(bdry_opr_2, 0, 1), bdry_opr_2)
    return lap_down
...
def torch_persistence_lap_up_v2(X, filt_1, filt_2, channel, cell_dim, device="cpu"):# filt_1 < filt_2 
    if (cell_dim == 1) or (cell_dim == 0):
        lap_up = torch_laplacian_up_coo_sparse(X=X, channel=channel, filt=filt_2, cell_dim=cell_dim, device=device) 
        ...
        perslap_up = torch.sparse_coo_tensor((lap_dim, lap_dim)).to(dtype=torch.float32, device=device)
...
def torch_persistence_laplacian_v2(X, filt_1, filt_2, channel, cell_dim):# filt_1 < filt_2 
    if (cell_dim == 1) or (cell_dim == 0):
        ...
        if cell_dim == 1:
            lap_down = torch_laplacian_down_coo_sparse(X=X, channel=channel, filt=filt_1, cell_dim=cell_dim)
            perslap = perslap_up + lap_down.to_dense()
        elif cell_dim == 0:
            perslap = perslap_up

    elif cell_dim == 2:
        perslap = torch_laplacian_down_coo_sparse(X=X, channel=channel, filt = filt_1, cell_dim=cell_dim)
    return perslap
...
def Schur_cmpl_v2(M, filt_idx):
    if M.is_sparse:
        M = M.to_dense()
    try:
        schur = M[filt_idx, :][:, filt_idx] - M[filt_idx, :][:, ~filt_idx] @ torch.linalg.pinv(M[~filt_idx, :][:, ~filt_idx], hermitian= True) @ M[~filt_idx, :][:, filt_idx]
    except:
        d = len(filt_idx)
        schur = torch.full([d, d], torch.inf)

    if schur.numel() == 0:
        schur = torch.tensor([[0.]])
    return schur

def cell_idx_check(cell_filt_1, cell_filt_2):
    if cell_filt_1.numel() == 0 or cell_filt_2.numel() == 0:
...
def torch_persistence_lap_up_filtration_v2(X, channel, cell_dim, start=0., end=1.0, steps=5):
    ...
    for t in range(len(times)):
        if t == 0:
            lap_up = torch_laplacian_up_coo_sparse(X=X, channel=channel, filt=end, cell_dim=cell_dim)
        else:
...
def torch_persistence_lap_down_filtration_v2(X, channel, cell_dim, start=0., end=1.0, steps=5):
    ...
    for t in range(len(times)):
        lap_down = torch_laplacian_down_coo_sparse(X=X, channel=channel, filt=times[t], cell_dim=cell_dim)
        yield lap_down
...
def bdry_opr_row_simplyfying(bd_opr): # bd_opr is a coo sparse object, reduce all 0 rows
    rows = bd_opr._indices()[0, :]
    rows_uniq = torch.unique(rows)
    all_rows = rows_uniq.unsqueeze(0) == rows.unsqueeze(1)
    rows_simplify = all_rows.nonzero()[:, 1]

    cols = bd_opr._indices()[1, :]
    
    indices = torch.stack([rows_simplify, cols])

    bdry_opr_simplified = torch.sparse_coo_tensor(indices, 
                                                  bd_opr._values(), (rows_uniq.shape[0], bd_opr.shape[1]))
    
    return bdry_opr_simplified
